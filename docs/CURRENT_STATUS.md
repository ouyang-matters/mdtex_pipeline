...
(41 checks), `node scripts/workflow-check.js` (37 steps) and
...
| **Loading screen** | A named-step loading screen from the first paint — before the bundle has parsed — while the application starts. It says which step it is on rather than spinning, and removes itself when ready. |
...
- `npm run check:data-safety` — populates an installation with articles, assets,
  ...
  verifies every category of user data byte for byte. 41 checks.
