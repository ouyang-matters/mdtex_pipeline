...
2. The web UI loads themes from `themes/builtin/` at build time; user themes from `~/.local/share/publisher/themes/` are only available via CLI (the browser cannot access the filesystem). A future improvement could add a theme upload/import feature in the UI.
3. KaTeX HTML may have alignment issues on WeChat for complex formulas
4. WeChat images must be uploaded to WeChat CDN manually (Phase 2 will add API upload)
5. The `publisher` CLI wrapper is installed to `~/.local/bin/` which may not be in PATH on all systems
