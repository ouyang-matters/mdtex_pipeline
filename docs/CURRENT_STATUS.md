...
Chrome 146). All of `npx vitest run` (372 tests), `node scripts/e2e.js`
(32 checks), `node scripts/workflow-check.js` (30 steps) and
...
- `node scripts/workflow-check.js` — walks the whole intended journey in order,
  ...
  copy the finished rich text. 30 steps, all passing on this machine.
