...
| `latex/` | Environment detection, Markdown → LaTeX, PDF templates, document assembly, CJK planning |
...
| `checkpoints.js` | Per-article snapshots, including the source container |
| `latex-source.js` | The LaTeX face of a Markdown article, and adopting it |
...
what reached the page. See `core/latex/cjk.js`.

**An article has one source.** A Markdown article can be *read* as LaTeX, and
that view is generated by the same builder the PDF build uses, so it cannot
disagree with what compiles. It cannot be edited, because Markdown maps into
LaTeX and LaTeX does not map back — two editable copies could only diverge.
Adopting LaTeX moves the source rather than duplicating it: `main.tex` is
written, `source.md` is removed, and the Markdown lives on in a checkpoint that
restores the container along with the content.
